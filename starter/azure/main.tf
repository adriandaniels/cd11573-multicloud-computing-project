...
resource "azurerm_container_group" "udacity" {
  ...
  dns_name_label      = "udacity-adaniels-azure"
  os_type             = "Linux"

  container {
    name   = "azure-container-app"
    # image  = "docker.io/tscotto5/azure_app:1.0"
    image = "adanielsregistry.azurecr.io/tscotto5/azure_app:1.0"
    cpu    = "0.5"
    memory = "1.5"
    environment_variables = {
      "AWS_S3_BUCKET"       = "udacity-adaniels-aws-s3-bucket",
      "AWS_DYNAMO_INSTANCE" = "udacity-adaniels-aws-dynamodb"
    }
    ports {
      port     = 3000
      protocol = "TCP"
